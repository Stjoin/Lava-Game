...
local Table = require(script.Parent.TableUtil)
...
function Signal.__index:Fire(...)
	...
	local Stack = pack(...)
	local NumConnectionsAndThreads = #self.Connections + (self.YieldingThreads or 0)

	Stack.NumConnectionsAndThreads = NumConnectionsAndThreads

	self.NextId = Id + 1
	self.Arguments[Id] = Stack
	self.YieldingThreads = 0  -- Reset to 0 instead of nil
...
function Signal.__index:Wait()
	self.YieldingThreads = (self.YieldingThreads or 0) + 1
	local Id = self.Bindable.Event:Wait()
	return GetArguments(self, Id)
end
